...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white px-6 py-4 shadow-md font-sans relative z-50">
      <div className="container mx-auto flex items-center justify-between relative">
        ...
        <ul className="hidden md:flex items-center space-x-4 text-sm relative">
          ...
          <li className="relative group">
            <span className="cursor-pointer text-gray-700 hover:bg-gray-100 px-3 py-2 rounded transition whitespace-nowrap">
              О ФАКУЛЬТЕТЕ
            </span>
            <ul className="absolute left-0 top-full mt-1 w-56 bg-white border border-gray-200 rounded shadow-md opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-opacity duration-200 z-20">
          ...
          {menuItems.map(({ name, path }) => (
            <li key={path}>
              <Link
                to={path}
                className="text-gray-700 hover:bg-gray-100 px-3 py-2 rounded transition whitespace-nowrap"
              >
                {name}
              </Link>
            </li>
        ...
        <div className="flex items-center space-x-4">
          <button
            onClick={toggleLang}
            className="hidden md:flex items-center space-x-2 hover:bg-gray-100 px-3 py-1 rounded text-gray-700"
      ...
      {/* Mobil menyu */}
      <div
        className={`md:hidden bg-white w-full shadow absolute left-0 right-0 transition-all duration-300 ease-in-out z-40 ${
          isOpen ? "top-16 opacity-100 visible" : "top-[-1000px] opacity-0 invisible"
        }`}
      >
